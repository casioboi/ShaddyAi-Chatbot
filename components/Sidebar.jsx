...
const Sidebar = ({ expand, setExpand }) => {
      ...
      <div>
        <div className={`flex ${expand ? 'flex-row gap-10' : 'flex-col items-center gap-8'}`}>
          ...
          >
            ...
            />
            <div className='absolute text-[10px] text-white bg-black px-1 py-[2px] rounded top-10 opacity-0 group-hover:opacity-100 transition'>
           ...
           <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? "bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max" : "group relative h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
            ...
            <div className='absolute w-max -top-12 -right-12 opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3
  ...
  {/* QR Code Popup */}
  <div className={`absolute -top-60 pb-8 ${!expand && "-right-40"} opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition duration-300`}>
    <div className='relative w-max bg-black text-white text-sm p-3 rounded-lg shadow-lg'>
      <Image src={assets.qrcode} alt='' className='w-44' />
      <p>Scan to get ShaddyAi App</p>
      <div className={`w-3 h-3 absolute bg-black rotate-45 ${expand ? "right-1/2" : "left-4"} -bottom-1.5`}></div>
    </div>
  </div>

  {/* Text and icon when expanded */}
  {expand && (
    <>
      <span>Get App</span>
      <Image alt='' src={assets.new_icon} className='w-10' />
    </>
  )}
</div>
